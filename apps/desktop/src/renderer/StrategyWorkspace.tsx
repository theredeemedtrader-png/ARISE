...
type PackageSelection = NonNullable<Awaited<ReturnType<AriseApi['selectStrategyPackage']>>>;
...
export function StrategyWorkspace() {
  ...
  const selectPackage=async()=>{if(!window.arise)return;setPackageBusy(true);try{const selected=await window.arise.selectStrategyPackage();if(selected)setPackageSelection(selected);setError(null);}catch(e){setError(e instanceof Error?e.message:String(e));}finally{setPackageBusy(false);}};
  const importPackage=async()=>{if(!window.arise||!packageSelection||!packageSelection.preview.checksum)return;setPackageBusy(true);try{const result=await window.arise.importStrategyPackage({previewToken:packageSelection.previewToken,expectedChecksum:packageSelection.preview.checksum,allowUpgrade:packageSelection.preview.disposition==='UPGRADE'});await reload();if(result.definitionId)setSelectedStrategyId(result.definitionId);if(result.mapId)setSelectedMapId(result.mapId);setPackageSelection(null);setError(result.status==='ALREADY_IMPORTED'?'Identical package version is already imported.':null);}catch(e){setError(e instanceof Error?e.message:String(e));}finally{setPackageBusy(false);}};
  const exportPackage=async(targetType:'STRATEGY'|'MAP',targetId:string)=>{if(!window.arise)return;setPackageBusy(true);try{const result=await window.arise.exportStrategyPackage({targetType,targetId});if(!result.canceled)setError(`Package exported: ${result.filePath}`);}catch(e){setError(e instanceof Error?e.message:String(e));}finally{setPackageBusy(false);}};
  ...
  return <div className="strategy-workspace">
    <header className="strategy-topline">
      ...
      <div className="strategy-top-actions">
        ...
        <button className="secondary-button" disabled={packageBusy} onClick={()=>void selectPackage()}>IMPORT PACKAGE</button>
        <button className="secondary-button" disabled={packageBusy||(!selectedMap&&!selectedStrategy)} onClick={()=>selectedMap?void exportPackage('MAP',selectedMap.mapId):selectedStrategy?void exportPackage('STRATEGY',selectedStrategy.definitionId):undefined}>EXPORT PACKAGE</button>
    ...
    {packageSelection?<div className="strategy-modal-backdrop"><section className="strategy-modal package-preview" aria-label="Import ARISE Strategy Package">
      <header><div><span>IMPORT ARISE {packageSelection.preview.packageType??'PACKAGE'}</span><strong>{packageSelection.preview.name??'Package validation failed'}</strong></div><button aria-label="Close package preview" onClick={()=>setPackageSelection(null)}>×</button></header>
      <div className="package-version">Version {packageSelection.preview.version??'—'} <b>{packageSelection.preview.disposition??'INVALID'}</b></div><p>{packageSelection.preview.description}</p>
      <div className="package-preview-grid">
        <section><span>AUTHOR / SOURCE</span><strong>{packageSelection.preview.author??'—'}</strong><small>{packageSelection.preview.source??'—'}</small></section>
        <section><span>INTEGRITY</span><strong>{packageSelection.preview.integrity}</strong><small className="checksum">{packageSelection.preview.checksum??'No verified checksum'}</small></section>
        <section><span>GRAPH</span><strong>{packageSelection.preview.graphSummary.nodes} nodes · {packageSelection.preview.graphSummary.edges} edges</strong><small>{packageSelection.preview.graphSummary.actions.length?`Actions: ${packageSelection.preview.graphSummary.actions.join(', ')}`:'No actions'}</small></section>
        <section><span>DEPLOYMENT</span><strong>{packageSelection.preview.deploymentModes.join(' · ')||'NONE'}</strong><small>Imports in OBSERVE · LIVE unavailable</small></section>
      </div>
      <div className="package-preview-list"><span>DEPENDENCIES</span>{packageSelection.preview.dependencies.length?packageSelection.preview.dependencies.map((dependency)=><div key={`${dependency.kind}-${dependency.id}`} className={dependency.status==='SATISFIED'?'pass':'fail'}><b>{dependency.status==='SATISFIED'?'✓':'✕'}</b><strong>{dependency.kind} · {dependency.id}</strong><small>{dependency.versionRange}{dependency.installedVersion?` · installed ${dependency.installedVersion}`:''}</small></div>):<small>No external dependencies</small>}</div>
      <div className="package-preview-list"><span>PARAMETERS / TIMEFRAMES / EVIDENCE</span><small>{packageSelection.preview.parameters.map((item)=>`${item.label} (${item.type})`).join(', ')||'No parameters'}</small><small>{packageSelection.preview.timeframeMappings.map((item)=>`${item.timeframe}: ${item.purposes.join('+')||'none'} ${item.importance}`).join(' · ')||'No timeframe mappings'}</small><small>{packageSelection.preview.evidencePolicy?`${packageSelection.preview.evidencePolicy.profile}: ${packageSelection.preview.evidencePolicy.events.join(', ')}`:'No evidence policy'}</small></div>
      {packageSelection.preview.warnings.map((message)=><div className="package-warning" key={message}>WARNING · {message}</div>)}{packageSelection.preview.errors.map((message)=><div className="package-error" key={message}>ERROR · {message}</div>)}
      <div className="package-safety"><b>{packageSelection.preview.valid?'✓':'✕'} DATA-ONLY VALIDATION</b><span>No executable code · no broker command · no LIVE authority</span></div>
      <footer><button onClick={()=>setPackageSelection(null)}>CANCEL</button><button className="primary-button" disabled={!packageSelection.preview.valid||packageBusy||!packageSelection.preview.checksum} onClick={()=>void importPackage()}>{packageSelection.preview.disposition==='UPGRADE'?'CONFIRM UPGRADE':packageSelection.preview.disposition==='IDENTICAL'?'VERIFY IMPORT':'IMPORT'}</button></footer>
    </section></div>:null}
  </div>;
}
